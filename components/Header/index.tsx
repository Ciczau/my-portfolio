...
const Header = () => {
    ...
    useEffect(() => {
        ...
        const isVisible = rect.top <= windowHeight - visibleThreshold && rect.bottom >= visibleThreshold;
                  if(isVisible){
                    setVisible(el.url)
                  }
            }
        ...
        <S.Wrapper>
            ...
            <S.MenuLine percentage={viewPercentage}/>
                ...
                {MenuItems.map((element, index) => {
                    return (
                        <S.MenuElement key={index} onClick={() => handleScroll(element.url)} style={{fontWeight: visible === element.url && "bold"}}>
                            {element.name}
                        </S.MenuElement>
                    )
                })}
